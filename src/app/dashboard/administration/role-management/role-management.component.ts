...
export class RoleManagementComponent implements OnInit {
  ...
  dataSourceObservable: Observable<
    Map<Role, MatTableDataSource<RolePermissions>>
  > = this.roleManagementService.rolesAsObservable.pipe(
    map(rolePermissions => {
  ...
  getDatasource(): MatTableDataSource<RolePermissions> {
    return (
      this.dataSources.get(this.datasourceToUse) ??
      new MatTableDataSource<RolePermissions>()
    );
  }
