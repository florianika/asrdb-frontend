...
import {
  ...
  RolePermissions,
} from '../../../model/RolePermissions.model';
...
export class RoleManagementService {
    ...
    this.httpClient
      ...
      .subscribe({
        ...
        error: error => {
          ...
          console.error(error);
          this.showMessage(
            'Could not load the role permissions. Please reload the page to try again.'
          );
        },
      });
  }
  ...
  openCreateRoleDialog() {
    this.dialog
      .open(RoleCreateDialogComponent)
      .afterClosed()
      .subscribe((newRole: NewRolePermission) => {
        if (newRole) {
  ...
  openDeleteRoleDialog(role: RolePermissions) {
    this.dialog
      .open(RoleDeleteDialogComponent, { data: { role } })
      .afterClosed()
      .subscribe((id: number) => {
        if (id) {
          this.deleteRole(id);
  ...
  openUpdateRoleDialog(role: RolePermissions) {
    this.dialog
      .open(RoleEditDialogComponent, { data: { permission: role.permission } })
      .afterClosed()
    ...
    this.httpClient
      ...
      .subscribe({
        ...
        error: err => {
          ...
          this.loading.next(false);
          this.showMessage(
            'Could not create the role permissions. Please try again or contact the administrator.'
          );
        },
      });
  }
    ...
    this.httpClient
      ...
      .subscribe({
        ...
        error: err => {
          ...
          this.loading.next(false);
          this.showMessage(
            'Could not update the role permissions. Please try again or contact the administrator.'
          );
        },
      });
  }
  ...
  private deleteRole(id: number) {
    this.loading.next(true);
    this.httpClient
      .delete<any>(environment.base_url + '/admin/permissions/' + id)
      .subscribe({
        next: () => {
          this.loading.next(false);
          this.getRolePermissions();
        ...
        error: err => {
          ...
          this.loading.next(false);
          this.showMessage(
            'Could not delete the role permissions. Please try again or contact the administrator.'
          );
        },
      });
  }
