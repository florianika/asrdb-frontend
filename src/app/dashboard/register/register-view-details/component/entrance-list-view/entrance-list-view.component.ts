...
export class EntranceListViewComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  ngOnInit() {
    this.buildingIdQueryParam =
      this.activatedRoute.snapshot.queryParamMap.get('building');
    if (this.buildingIdQueryParam) {
      this.filterConfig.filter.EntBldGlobalID = this.buildingIdQueryParam
        ?.replace('{', '')
        .replace('}', '');
    } else if (this.buildingGlobalId) {
      this.filterConfig.filter.EntBldGlobalID = this.buildingGlobalId
        ?.replace('{', '')
        .replace('}', '');
    }
  }
    ...
    this.matDialog
      .open(EntranceListViewFilterComponent, {
        data: {
          ...
          showBuildingIdFilter: !this.buildingGlobalId,
        },
      })
      .afterClosed()
      .pipe(takeUntil(this.destroy$))
      .subscribe((newFilterConfig: EntranceFilter | null) =>
  ...
  viewEntranceDetails(globalId: string) {
    this.matDialog.open(EntranceDetailsComponent, {
      ...
      data: {
        ...
        logs: this.registerLogService
          .getAllLogs(ENTRANCE_ENTITY)
          .filter(log =>
            globalId.toLowerCase().includes(log.entId?.toLowerCase() as string)
          ),
      },
    });
  }
  ...
  private handleResponse(res: any) {
    ...
    const streetIds = attributes
      .map((el: any) => el.EntStrGlobalID?.replace('{', '').replace('}', ''))
      .filter((el: any) => !!el);
    if (!streetIds.length) {
      ...
      .subscribe(streetRes => {
        ...
        attributes.forEach((attribute: any) => {
          const street = streets.find(
            (streat: any) => streat.GlobalID === attribute.EntStrGlobalID
          );
